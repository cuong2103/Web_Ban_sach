...
<div class="relative overflow-hidden" id="banner-slider" style="min-height:340px;">
    ...
    $banners = [
        [
            ...
            'sub' => 'Hàng nghìn đầu sách chất lượng — giảm đến <strong>50%</strong> hôm nay',
            'cta' => 'Khám phá ngay',
            'badge' => '🔥 Flash Sale',
            'from' => '#1B5E20',
            'to' => '#4CAF50',
        ...
        [
            ...
            'sub' => 'Cập nhật hàng nghìn đầu sách mới nhất mỗi tuần',
            'cta' => 'Xem sách mới',
            'badge' => '✨ Mới về',
            'from' => '#0D47A1',
            'to' => '#1976D2',
        ...
        [
            ...
            'sub' => 'Giảm giá sốc chỉ trong 24 giờ — Đừng bỏ lỡ!',
            'cta' => 'Mua ngay',
            'badge' => '⚡ Giới hạn',
            'from' => '#BF360C',
            'to' => '#E64A19',
    ...
    foreach ($banners as $i => $b): ?>
        <div class="banner-slide absolute inset-0 flex flex-col justify-center px-8 md:px-20 transition-opacity duration-700 <?= $i === 0 ? 'opacity-100 relative' : 'opacity-0 pointer-events-none' ?>"
            ...
            <div class="relative z-10 max-w-xl">
                <span
                    class="inline-block bg-white/20 backdrop-blur-sm text-white text-xs font-semibold px-3 py-1 rounded-full mb-4">
                    <?= $b['badge'] ?>
                </span>
    ...
    <div class="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-2 z-10" id="banner-dots">
        <?php for ($i = 0; $i < count($banners); $i++): ?>
            <button
                class="banner-dot h-2 rounded-full transition-all duration-300 <?= $i === 0 ? 'bg-white w-8' : 'bg-white/50 w-2' ?>"></button>
        <?php endfor; ?>
    </div>
</div>
...
<div class="max-w-[1200px] mx-auto px-4 py-10 space-y-14">
    ...
    <section>
        <div
            class="bg-gradient-to-r from-red-500 via-orange-500 to-red-600 rounded-t-2xl px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-2.5">
                ...
                <span class="bg-white/20 text-white text-xs px-2 py-0.5 rounded-full ml-1">Hôm nay</span>
            </div>
            <div class="flex items-center gap-2 text-white" id="flash-timer">
                <i data-lucide="clock" class="w-4 h-4"></i>
                <span class="text-sm">Kết thúc trong:</span>
                <span class="bg-white/20 rounded-lg px-2.5 py-1 text-sm font-mono font-bold" id="t-h">05</span>
                <span class="font-bold">:</span>
                <span class="bg-white/20 rounded-lg px-2.5 py-1 text-sm font-mono font-bold" id="t-m">23</span>
                <span class="font-bold">:</span>
                <span class="bg-white/20 rounded-lg px-2.5 py-1 text-sm font-mono font-bold" id="t-s">45</span>
            </div>
        </div>
        <div class="bg-white rounded-b-2xl p-6 shadow-sm">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
                <?php foreach ($saleBooks as $book):
                    $pct = round((1 - $book['sale_price'] / $book['price']) * 100);
                    ?>
                    <div
                        class="group relative bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all hover:-translate-y-1 cursor-pointer">
                        <div class="absolute top-3 left-3 z-10">
                            <span class="bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm">-
                                <?= $pct ?>%
                            </span>
                        </div>
                        ...
                        <div class="p-4">
                            <p class="text-sm font-semibold text-[#333] truncate mb-0.5"><a
                                    href="<?= BASE_URL ?>?act=book-detail&id=<?= $book['id'] ?>">
    ...
    <section>
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="<?= BASE_URL ?>?act=books"
                class="text-sm text-[#4CAF50] font-medium hover:underline flex items-center gap-1">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
            <?php foreach ($newBooks as $book):
                ...
                ?>
                <div
                    class="group bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all hover:-translate-y-1">
                    <a href="<?= BASE_URL ?>?act=book-detail&id=<?= $book['id'] ?>"
                        class="relative block h-44 bg-gray-50 flex items-center justify-center overflow-hidden">
                        <span
                            class="absolute top-3 left-3 bg-[#4CAF50] text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-sm z-10">MỚI</span>
                        <img src="<?= !empty($book['thumbnail']) ? (str_starts_with($book['thumbnail'], 'http') ? htmlspecialchars($book['thumbnail']) : rtrim(BASE_URL, '/') . '/' . ltrim($book['thumbnail'], '/')) : 'https://placehold.co/300x400?text=No+Image' ?>"
                            onerror="this.onerror=null; this.src='https://placehold.co/300x400?text=No+Image';"
                            class="h-full object-cover group-hover:scale-105 transition-transform duration-500"
                            alt="<?= htmlspecialchars($book['title']) ?>">
    ...
    <section>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
            ...
            foreach ($hotBooks as $book):
                $displayPrice = $book['sale_price'] ?? $book['price'];
                // Giả lập lượng bán ngẫu nhiên nếu trong db chưa có trường sold thực
                $sold = rand(500, 1500) - ($rank * 100);
                ?>
                <div
                    class="group bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all hover:-translate-y-1">
                    <a href="<?= BASE_URL ?>?act=book-detail&id=<?= $book['id'] ?>"
                        class="relative block h-44 bg-gray-50 flex items-center justify-center overflow-hidden">
                        <span
                            class="absolute top-3 left-3 bg-[#FFC107] text-[#333] text-[10px] font-bold w-7 h-7 rounded-full flex items-center justify-center shadow-sm z-10">#
                            <?= $rank++ ?>
                        </span>
                        <img src="<?= !empty($book['thumbnail']) ? (str_starts_with($book['thumbnail'], 'http') ? htmlspecialchars($book['thumbnail']) : rtrim(BASE_URL, '/') . '/' . ltrim($book['thumbnail'], '/')) : 'https://placehold.co/300x400?text=No+Image' ?>"
                            class="h-full object-cover group-hover:scale-105 transition-transform duration-500"
    ...
    <section>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            <?php
            $categoryIcons = [
                'van-hoc' => ['icon' => '📖', 'color' => 'from-blue-50 to-blue-100', 'border' => 'border-blue-200', 'hover' => 'hover:border-blue-400'],
                'kinh-te' => ['icon' => '💼', 'color' => 'from-green-50 to-green-100', 'border' => 'border-green-200', 'hover' => 'hover:border-green-400'],
                'thieu-nhi' => ['icon' => '🧒', 'color' => 'from-yellow-50 to-yellow-100', 'border' => 'border-yellow-200', 'hover' => 'hover:border-yellow-400'],
                'ky-nang-song' => ['icon' => '🌟', 'color' => 'from-purple-50 to-purple-100', 'border' => 'border-purple-200', 'hover' => 'hover:border-purple-400'],
                'khoa-hoc' => ['icon' => '🔬', 'color' => 'from-cyan-50 to-cyan-100', 'border' => 'border-cyan-200', 'hover' => 'hover:border-cyan-400'],
                'lich-su' => ['icon' => '📜', 'color' => 'from-orange-50 to-orange-100', 'border' => 'border-orange-200', 'hover' => 'hover:border-orange-400'],
            ];
            $defaultIcon = ['icon' => '📚', 'color' => 'from-indigo-50 to-indigo-100', 'border' => 'border-indigo-200', 'hover' => 'hover:border-indigo-400'];
            ...
            if (!empty($categories)):
                foreach ($categories as $cat):
                    ...
                    <a href="<?= BASE_URL ?>?act=books&category=<?= urlencode($cat['slug']) ?>"
                        class="flex items-center gap-4 p-5 bg-gradient-to-r <?= $icons['color'] ?> border <?= $icons['border'] ?> <?= $icons['hover'] ?> rounded-2xl hover:shadow-md transition-all hover:-translate-y-0.5">
                        <span class="text-4xl">
                            <?= $icons['icon'] ?>
                        </span>
                        <div>
                ...
                $featCats = [
                    ['icon' => '📖', 'name' => 'Văn học', 'count' => '1,240 cuốn', 'slug' => 'van-hoc', 'color' => 'from-blue-50 to-blue-100', 'border' => 'border-blue-200', 'hover' => 'hover:border-blue-400'],
                    ['icon' => '💼', 'name' => 'Kinh tế', 'count' => '856 cuốn', 'slug' => 'kinh-te', 'color' => 'from-green-50 to-green-100', 'border' => 'border-green-200', 'hover' => 'hover:border-green-400'],
                    ['icon' => '🧒', 'name' => 'Thiếu nhi', 'count' => '620 cuốn', 'slug' => 'thieu-nhi', 'color' => 'from-yellow-50 to-yellow-100', 'border' => 'border-yellow-200', 'hover' => 'hover:border-yellow-400'],
                    ['icon' => '🌟', 'name' => 'Kỹ năng sống', 'count' => '743 cuốn', 'slug' => 'ky-nang-song', 'color' => 'from-purple-50 to-purple-100', 'border' => 'border-purple-200', 'hover' => 'hover:border-purple-400'],
                    ['icon' => '🔬', 'name' => 'Khoa học', 'count' => '512 cuốn', 'slug' => 'khoa-hoc', 'color' => 'from-cyan-50 to-cyan-100', 'border' => 'border-cyan-200', 'hover' => 'hover:border-cyan-400'],
                    ['icon' => '📜', 'name' => 'Lịch sử', 'count' => '389 cuốn', 'slug' => 'lich-su', 'color' => 'from-orange-50 to-orange-100', 'border' => 'border-orange-200', 'hover' => 'hover:border-orange-400'],
                ];
                foreach ($featCats as $cat): ?>
                    <a href="<?= BASE_URL ?>?act=books&category=<?= urlencode($cat['slug']) ?>"
                        class="flex items-center gap-4 p-5 bg-gradient-to-r <?= $cat['color'] ?> border <?= $cat['border'] ?> <?= $cat['hover'] ?> rounded-2xl hover:shadow-md transition-all hover:-translate-y-0.5">
                        <span class="text-4xl">
                            <?= $cat['icon'] ?>
                        </span>
                        <div>
    ...
    <section
        class="rounded-3xl overflow-hidden bg-gradient-to-br from-[#F9F9F9] to-white border border-gray-100 shadow-sm p-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
            ...
            foreach ($whys as $w): ?>
                <div class="text-center group">
                    <div
                        class="w-16 h-16 <?= $w['color'] ?> rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg group-hover:scale-110 transition-transform">
                        <i data-lucide="<?= $w['icon'] ?>" class="w-7 h-7 text-white"></i>
                    </div>
                    <h3 class="font-bold text-[#333] mb-1.5 text-sm">
                        <?= $w['title'] ?>
                    </h3>
    ...
    <!-- ── EMAIL SIGNUP ── -->
    <section
        class="rounded-3xl overflow-hidden bg-gradient-to-r from-[#1B5E20] via-[#2E7D32] to-[#4CAF50] p-10 flex flex-col md:flex-row items-center justify-between gap-8 relative">
        <div class="absolute right-0 top-0 w-64 h-64 rounded-full bg-white/5" style="transform:translate(20%, -30%)">
        </div>
        ...
        <div class="flex gap-2.5 w-full md:w-auto relative z-10">
            ...
            <button
                class="bg-[#FFC107] hover:bg-[#FFB300] text-[#333] px-6 py-3 rounded-xl font-bold transition-all hover:shadow-lg whitespace-nowrap">
                Đăng ký
            </button>
        </div>
    </section>
