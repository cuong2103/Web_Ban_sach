    ...
    <div class="w-full">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Tổng cộng <span class="font-semibold text-gray-700">
                        <?= number_format($total) ?>
                    </span> sách trong hệ thống
                </p>
            </div>
            <a href="<?= BASE_URL ?>?act=admin-books-create"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <form action="" method="GET" class="flex flex-wrap gap-3 items-center w-full">
                    ...
                    <div class="relative w-44">
                        <select name="category"
                            class="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#4CAF50]/20 focus:border-[#4CAF50] transition-colors text-gray-600 text-sm">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50/50 border-b border-gray-100">
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Tên sách
                            </th>
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Danh mục
                            </th>
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Ảnh</th>
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Giá /
                                Khuyến mãi</th>
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Kho</th>
                            <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Trạng
                                thái</th>
                            <th
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-14 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center gap-3">
                                        ...
                                        <a href="<?= BASE_URL ?>?act=admin-books-create"
                                            class="text-sm text-[#4CAF50] hover:underline font-medium">+ Thêm sách mới</a>
                                    </div>
                                </td>
                            </tr>
                        <?php else: ?>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <?php if ($book['thumbnail']): ?>
                                            ...
                                            <img src="<?= htmlspecialchars(BASE_URL . $book['thumbnail']) ?>?v=<?= file_exists($imgPath) ? filemtime($imgPath) : 0 ?>"
                                                alt="<?= htmlspecialchars($book['title']) ?>"
                                                class="w-11 h-14 object-cover rounded-lg shadow-sm border border-gray-200">
                                        <?php else: ?>
                                            <div
                                                class="w-11 h-14 bg-gray-100 rounded-lg flex items-center justify-center border border-gray-200 text-gray-400">
                                    ...
                                    <td class="px-6 py-4 text-sm">
                                        ...
                                        <?php else: ?>
                                            <span
                                                class="text-red-500 font-semibold bg-red-50 px-2 py-0.5 rounded-full text-xs">Hết</span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col gap-1.5">
                                            ...
                                            if (!empty($_GET['search']))
                                                $toggleUrl .= '&search=' . urlencode($_GET['search']);
                                            if (!empty($_GET['category']))
                                                $toggleUrl .= '&category=' . urlencode($_GET['category']);
                                            if (!empty($_GET['page']))
                                                $toggleUrl .= '&page=' . (int) $_GET['page'];
                                            ?>
                                            <a href="<?= $toggleUrl ?>"
                                                ...
                                                <i data-lucide="<?= $book['status'] == 1 ? 'eye' : 'eye-off' ?>"
                                                    class="w-3 h-3"></i>
                                            ...
                                            <?php if (!empty($book['is_featured']) && $book['is_featured'] == 1): ?>
                                                <span
                                                    class="inline-flex w-fit items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-yellow-100 text-yellow-800 border border-yellow-200">
                                            ...
                                            <?php if (!empty($book['is_bestseller']) && $book['is_bestseller'] == 1): ?>
                                                <span
                                                    class="inline-flex w-fit items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 text-blue-800 border border-blue-200">
            ...
            <?php if (!empty($totalPages) && $totalPages > 1): ?>
                <div
                    class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 flex flex-wrap items-center justify-between gap-4">
                    <div class="text-sm text-gray-500">
                        Trang <span class="font-semibold text-gray-900">
                            <?= $page ?>
                        </span> /
                        <?= $totalPages ?>
                    ...
                    <div class="flex gap-1">
                        ...
                        if (!empty($_GET['category']))
                            $baseUrl .= "&category=" . urlencode($_GET['category']);
                        if (isset($_GET['status_filter']) && $_GET['status_filter'] !== '')
                            $baseUrl .= "&status_filter=" . urlencode($_GET['status_filter']);
                        ?>

                        <?php if ($page > 1): ?>
                            <a href="<?= $baseUrl ?>&page=1"
                                class="px-3 py-1.5 border border-gray-200 rounded-lg bg-white text-gray-600 hover:bg-gray-50 text-sm hidden sm:block">Đầu</a>
                            <a href="<?= $baseUrl ?>&page=<?= $page - 1 ?>"
                                class="px-3 py-1.5 border border-gray-200 rounded-lg bg-white text-gray-600 hover:bg-gray-50 flex items-center text-sm">
                        ...
                        <?php if ($page < $totalPages): ?>
                            ...
                            <a href="<?= $baseUrl ?>&page=<?= $totalPages ?>"
                                class="px-3 py-1.5 border border-gray-200 rounded-lg bg-white text-gray-600 hover:bg-gray-50 text-sm hidden sm:block">Cuối</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
